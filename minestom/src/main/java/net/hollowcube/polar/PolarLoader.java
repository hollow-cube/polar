...
public class PolarLoader implements IChunkLoader {
    ...
    public PolarLoader(@NotNull Path path) throws IOException {
        this(path, Files.exists(path) ? MinestomPolarReader.read(Files.readAllBytes(path)) : new PolarWorld());
    }
    ...
    public PolarLoader(@NotNull InputStream inputStream) throws IOException {
        try (inputStream) {
            this.worldData = MinestomPolarReader.read(inputStream.readAllBytes());
            this.savePath = null;
        }
    }
